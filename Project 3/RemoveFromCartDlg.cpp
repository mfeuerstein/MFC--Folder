...
IMPLEMENT_DYNAMIC(RemoveFromCartDlg, CDialogEx)

RemoveFromCartDlg::RemoveFromCartDlg(CWnd* pParent /*=NULL*/)
: CDialogEx(RemoveFromCartDlg::IDD, pParent)
{
	index = -1;
	itemQuant = -1;
...
RemoveFromCartDlg::RemoveFromCartDlg(const LinkList list, CWnd* pParent)
	: CDialogEx(RemoveFromCartDlg::IDD, pParent)
{
	inventory = list;
}

RemoveFromCartDlg::~RemoveFromCartDlg()
{
}

void RemoveFromCartDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(RemoveFromCartDlg, CDialogEx)
	ON_WM_ACTIVATE()
ON_BN_CLICKED(IDD_REMOVE_ITEM_BTTN, &RemoveFromCartDlg::OnClickedIddRemoveItemBttn)
ON_LBN_SELCHANGE(IDC_LIST_RFC_ITEMS, &RemoveFromCartDlg::OnSelchangeListRfcItems)
ON_LBN_SELCHANGE(IDC_LIST_RFC_QUANT, &RemoveFromCartDlg::OnSelchangeListRfcQuant)
ON_CBN_SELCHANGE(IDC_LIST_RFC_ITEMS, &RemoveFromCartDlg::OnSelchangeListRfcItems)
//ON_COMMAND(IDC_LIST_ATC_QUANT, &RemoveFromCartDlg::OnListAtcQuant)
ON_CBN_SELCHANGE(IDC_LIST_RFC_QUANT, &RemoveFromCartDlg::OnSelchangeListRfcQuant)
END_MESSAGE_MAP()


// AddToCartDlg message handlers

void RemoveFromCartDlg::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	CDialogEx::OnActivate(nState, pWndOther, bMinimized);
}

string RemoveFromCartDlg::getStockID()
{
	return inventory.getProductAt(index).stockID;
}

int RemoveFromCartDlg::getQuantity()
{
	return itemQuant;
}

int RemoveFromCartDlg::getIndex()
{
	return index;
}


void RemoveFromCartDlg::OnClickedIddRemoveItemBttn()
{
	EndDialog(IDOK);
}


/*void RemoveFromCartDlg::OnSelchangeListRfcItems()
{
...
void RemoveFromCartDlg::OnSelchangeListRfcQuant()
{
